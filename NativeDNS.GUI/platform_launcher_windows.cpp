...
bool launchNativeDnsCore(const QString& executable,const QStringList& arguments,bool elevated,QString* error){
    ...
    if(!ShellExecuteExW(&info)){
        if(error)*error=shellError(GetLastError());
        return false;
    }
...
bool runNativeDnsHelper(const QString& executable,const QStringList& arguments,bool elevated,int* exitCode,QString* error){
    ...
    if(!info.hProcess){
        if(error)*error="Elevated helper did not return a process handle.";
        return false;
    }
    ...
    if(wait!=WAIT_OBJECT_0){
        ...
        if(error)*error=QString("Waiting for elevated helper failed: %1").arg(code);
        return false;
    }
    ...
    if(!GetExitCodeProcess(info.hProcess,&code)){
        ...
        CloseHandle(info.hProcess);
        if(error)*error=QString("Cannot read elevated helper exit code: %1").arg(winError);
        return false;
    }
    CloseHandle(info.hProcess);
