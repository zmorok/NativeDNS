...
bool launchNativeDnsCore(const QString& executable,
    ...
    if (elevated) {
        ...
        if (pkexec.isEmpty()) {
            if (error)
                *error = "pkexec is required for transparent DNS interception";
            return false;
        }
    ...
    if (!QProcess::startDetached(program, args)) {
        if (error)
            *error = "Cannot start NativeDNSCoreHost";
        return false;
    }
    return true;
}
...
bool runNativeDnsHelper(const QString& executable,
    ...
    if (elevated) {
        ...
        if (pkexec.isEmpty()) {
            if (error)
                *error = "pkexec is required for elevated NativeDNS helper operations";
            return false;
        }
        args.prepend(executable);
        program = pkexec;
