...
[[noreturn]] void gui_terminate() noexcept {
    ...
    if(const auto failure=std::current_exception())try{std::rethrow_exception(failure);}catch(const std::exception& error){message=error.what();}catch(...){message="GUI terminated with an unknown exception";}
    try{nd::Logger logger(8);logger.configure_file(true,nd::Level::errors_only,nd::timestamped_log_path(nd::platform::application_root_directory()/"logs"));logger.write(nd::Level::errors_only,"GUI_TERMINATED",message);}catch(...){}
    std::abort();
}
}
