...
uint16_t be16(std::span<const uint8_t> bytes, size_t at) {
    if (at + 2 > bytes.size())
        throw Error("DNS_MALFORMED", "Truncated DNS field");
    return static_cast<uint16_t>((bytes[at] << 8) | bytes[at + 1]);
}
uint32_t be32(std::span<const uint8_t> bytes, size_t at) {
    if (at + 4 > bytes.size())
        throw Error("DNSCRYPT_CERT", "Truncated DNSCrypt certificate");
...
size_t skip_name(std::span<const uint8_t> packet, size_t at) {
    for (size_t labels = 0; labels <= 127; ++labels) {
        if (at >= packet.size())
            throw Error("DNS_MALFORMED", "Truncated DNS name");
        const uint8_t length = packet[at++];
        if ((length & 0xc0) == 0xc0) {
            if (at >= packet.size())
                throw Error("DNS_MALFORMED", "Truncated DNS compression pointer");
            const size_t pointer = static_cast<size_t>((length & 0x3f) << 8) | packet[at];
            if (pointer < 12 || pointer >= at - 1)
                throw Error("DNS_MALFORMED", "Invalid DNS compression pointer");
            return at + 1;
        }
        if (length & 0xc0)
            throw Error("DNS_MALFORMED", "Invalid DNS label encoding");
        if (!length)
            return at;
        if (length > 63 || at + length > packet.size())
            throw Error("DNS_MALFORMED", "Invalid DNS label length");
        at += length;
    }
...
Server relay_endpoint(const Server& server) {
    ...
    if (value.empty())
        throw Error("ENDPOINT", "Anonymized DNSCrypt requires a relay endpoint");
    if (value.front() == '[') {
        const auto close = value.find(']');
        if (close == std::string::npos)
            throw Error("ENDPOINT", "Invalid relay IPv6 endpoint");
        relay.ip = value.substr(1, close - 1);
        if (close + 1 < value.size()) {
            if (value[close + 1] != ':')
                throw Error("ENDPOINT", "Invalid relay endpoint");
...
Packet padded_certificate_request(Packet request) {
    ...
    if (request.size() + opt_overhead > target)
        throw Error("DNSCRYPT_SIZE", "DNSCrypt certificate name is too large to pad");
    ...
    request.insert(request.end(),
                   {0,
                    ...
                    0,
                    0,
                    static_cast<uint8_t>((padding + 4) >> 8),
                    static_cast<uint8_t>(padding + 4),
                    0,
                    12,
                    static_cast<uint8_t>(padding >> 8),
                    static_cast<uint8_t>(padding)});
    request.insert(request.end(), padding, 0);
    return request;
}
std::vector<Packet> fetch_certificates(const Server& server,
    ...
    const auto exchange = [&](bool through_relay) {
        const auto outgoing =
            through_relay ? build_anonymized_dnscrypt_certificate_packet(request, server) : request;
        const auto target = through_relay ? relay_endpoint(server) : endpoint(server);
        try {
            return detail::exchange_network(outgoing, target, false, deadline);
        } catch (const Error& error) {
            if (!retryable(error))
                throw;
            return detail::exchange_network(outgoing, target, true, deadline);
        }
    };
    Packet response;
    try {
        response = exchange(anonymized);
    } catch (const Error&) {
        if (!anonymized || !server.allow_direct_certificate_fallback)
            throw;
    ...
    if (parsed.truncated) {
        const auto outgoing =
            anonymized ? build_anonymized_dnscrypt_certificate_packet(request, server) : request;
    ...
    if (parsed.rcode)
        throw Error("DNSCRYPT_CERT",
                    "Certificate lookup failed with RCODE " + std::to_string(parsed.rcode));
...
public:
    ...
    Packet exchange(const Packet& request, const Server& server) override {
        ...
        if (expected.flags & 0x8000)
            throw Error("DNS_MALFORMED", "Expected query, not response");
        const auto deadline =
            detail::NetworkClock::now() + std::chrono::milliseconds(server.timeout_ms);
        ...
        } wipe_secret{client_secret.data(), client_secret.size()};
        if (crypto_box_curve25519xchacha20poly1305_keypair(client_public.data(),
        ...
        auto perform = [&](bool tcp) {
            ...
            if (tcp) {
                ...
                const size_t maximum_padding =
                    std::min<size_t>(256, 4096 - query_overhead - request.size());
                if (minimum_padding > maximum_padding)
                    throw Error("DNSCRYPT_SIZE",
                                "DNSCrypt TCP query cannot be padded within protocol bounds");
                const auto choices =
                    static_cast<uint32_t>(1 + (maximum_padding - minimum_padding) / 64);
                target = query_overhead + request.size() + minimum_padding +
                         static_cast<size_t>(randombytes_uniform(choices)) * 64;
            }
            auto encrypted = build_dnscrypt_query(request, cert, client_secret, nonce, target);
            Packet outgoing =
                anonymized_ ? build_anonymized_dnscrypt_packet(encrypted, server) : encrypted;
        ...
        if (parsed.truncated)
            throw Error("DNS_TRUNCATED", "Truncated DNSCrypt response over TCP");
        return response;
    }
...
private:
    ...
    static uint64_t unix_time() {
        const auto seconds = std::chrono::duration_cast<std::chrono::seconds>(
                                 ...
                                 .count();
        if (seconds < 0)
            throw Error("DNSCRYPT_CERT_TIME", "System clock predates the Unix epoch");
        return static_cast<uint64_t>(seconds);
    }
    DnsCryptCertificate load_certificate(const Server& server,
                                         detail::NetworkClock::time_point deadline) {
        const std::string key = protocol_name(server.protocol) + '|' + server.ip + ':' +
        ...
        std::optional<DnsCryptCertificate> current;
        {
            ...
            if (cache_.size() >= 128)
                for (auto entry = cache_.begin(); entry != cache_.end() && cache_.size() >= 128;) {
                    if (entry->second.use_count() == 1 && !entry->second->refreshing)
                        entry = cache_.erase(entry);
                    else
                        ++entry;
                }
            if (cache_.size() >= 128 && !cache_.contains(key))
                throw Error("DNSCRYPT_CERT_CACHE", "DNSCrypt certificate cache is full");
            auto& slot = cache_[key];
            if (!slot)
                slot = std::make_shared<CertificateState>();
            state = slot;
            for (;;) {
                const auto now = unix_time();
                const auto steady = detail::NetworkClock::now();
                const bool valid = state->certificate && now >= state->certificate->valid_from &&
                                   now < state->certificate->valid_until;
                if (valid && steady < state->refresh_after)
        ...
        try {
            ...
            auto certificate = select_dnscrypt_certificate(
                fetch_certificates(server, request, parse_question(request), deadline),
                parse_dnscrypt_provider_key(server.public_key),
                now);
            {
                std::lock_guard lock(cache_mutex_);
                state->certificate = certificate;
                const auto refresh_seconds =
                    std::min<uint64_t>(3600, certificate.valid_until - now);
                state->refresh_after =
                    detail::NetworkClock::now() + std::chrono::seconds(refresh_seconds);
...
Packet build_anonymized_dnscrypt_packet(const Packet& payload, const Server& server) {
    ...
    bool ipv6 = false;
    if (!platform::parse_ip(server.ip, address, ipv6))
        throw Error("ENDPOINT", "Anonymized DNSCrypt target requires a numeric IP");
    if (!ipv6) {
        address[10] = 0xff;
        address[11] = 0xff;
    }
...
Packet build_anonymized_dnscrypt_certificate_packet(const Packet& request, const Server& server) {
    return build_anonymized_dnscrypt_packet(padded_certificate_request(request), server);
}
...
std::array<uint8_t, 32> parse_dnscrypt_provider_key(const std::string& text) {
    ...
    for (unsigned char c : text) {
        if (c == ':' || c == '-' || c == ' ' || c == '\t')
            continue;
        if (!std::isxdigit(c))
            throw Error("DNSCRYPT_KEY", "Provider key contains a non-hexadecimal character");
        hex.push_back(static_cast<char>(c));
    }
    if (hex.size() != 64)
        throw Error("DNSCRYPT_KEY", "Provider public key must contain 32 bytes");
    std::array<uint8_t, 32> result{};
    for (size_t i = 0; i < result.size(); ++i) {
        const auto digit = [](char c) -> uint8_t {
            return static_cast<uint8_t>(c <= '9' ? c - '0' : (c | 32) - 'a' + 10);
        };
...
void validate_dnscrypt_server(const Server& server) {
    ...
    if (server.ip.empty())
        throw Error("ENDPOINT", "DNSCrypt server requires a numeric IP");
    (void)normalize_host(server.provider_name);
    (void)parse_dnscrypt_provider_key(server.public_key);
    if (server.protocol == Protocol::anonymized_dnscrypt)
        (void)relay_endpoint(server);
}

DnsCryptCertificate select_dnscrypt_certificate(const std::vector<Packet>& blobs,
                                                std::span<const uint8_t, 32> provider_key,
                                                uint64_t unix_time) {
    sodium_ready();
    bool found = false, time_invalid = false;
    DnsCryptCertificate selected;
    for (const auto& blob : blobs) {
        if (blob.size() < classical_certificate_size ||
            !std::equal(cert_magic.begin(), cert_magic.end(), blob.begin()))
            continue;
        if (blob[4] != 0 || (blob[5] != 1 && blob[5] != 2) || blob[6] != 0 || blob[7] != 0)
            continue;
        if (crypto_sign_verify_detached(blob.data() + 8,
                                        blob.data() + 72,
                                        static_cast<unsigned long long>(blob.size() - 72),
                                        provider_key.data()) != 0)
            continue;
        ...
        candidate.serial = be32(blob, 112);
        candidate.valid_from = be32(blob, 116);
        candidate.valid_until = be32(blob, 120);
        if (candidate.encryption_system == 2 && std::all_of(candidate.client_magic.begin(),
                                                            candidate.client_magic.begin() + 7,
                                                            [](uint8_t byte) { return byte == 0; }))
            continue;
        if (candidate.valid_from >= candidate.valid_until)
            continue;
        if (unix_time < candidate.valid_from || unix_time >= candidate.valid_until) {
            time_invalid = true;
            continue;
        }
        if (!found || candidate.serial > selected.serial) {
            selected = candidate;
            found = true;
        }
    }
    if (!found && time_invalid)
        throw Error("DNSCRYPT_CERT_TIME",
                    "Authenticated DNSCrypt certificate is not valid at system time " +
                        std::to_string(unix_time));
    if (!found)
        throw Error("DNSCRYPT_CERT", "No valid supported DNSCrypt certificate");
    return selected;
}

std::vector<Packet> extract_dnscrypt_certificates(const Packet& response) {
    if (response.size() < 12)
        throw Error("DNS_MALFORMED", "Short DNS certificate response");
    size_t at = 12;
    const auto questions = be16(response, 4), answers = be16(response, 6);
    for (uint16_t i = 0; i < questions; ++i) {
        at = skip_name(response, at);
        if (at + 4 > response.size())
            throw Error("DNS_MALFORMED", "Truncated DNS question");
        at += 4;
    }
    std::vector<Packet> result;
    for (uint16_t i = 0; i < answers; ++i) {
        at = skip_name(response, at);
        if (at + 10 > response.size())
            throw Error("DNS_MALFORMED", "Truncated DNS answer");
        const auto type = be16(response, at), klass = be16(response, at + 2),
                   length = be16(response, at + 8);
        at += 10;
        if (at + length > response.size())
            throw Error("DNS_MALFORMED", "Truncated DNS RDATA");
        if (type == 16 && klass == 1) {
            Packet certificate;
            size_t part = at;
            while (part < at + length) {
                const size_t chunk = response[part++];
                if (part + chunk > at + length)
                    throw Error("DNS_MALFORMED", "Invalid TXT character-string");
                certificate.insert(certificate.end(),
                                   response.begin() + static_cast<std::ptrdiff_t>(part),
    ...
    if (result.empty())
        throw Error("DNSCRYPT_CERT", "Certificate response contains no TXT certificate");
    return result;
}
...
std::array<uint8_t, 32> dnscrypt_shared_key(std::span<const uint8_t, 32> client_secret,
    ...
    const int result =
        certificate.encryption_system == 1
            ? crypto_box_curve25519xsalsa20poly1305_beforenm(
                  shared.data(), certificate.resolver_public_key.data(), client_secret.data())
...
Packet build_dnscrypt_query(const Packet& request,
                            const DnsCryptCertificate& certificate,
                            std::span<const uint8_t, 32> client_secret,
                            std::span<const uint8_t, 12> client_nonce,
    ...
    const size_t needed = std::max(
        request.size() + 1,
        minimum_wire_size > query_overhead ? minimum_wire_size - query_overhead : size_t{1});
    const size_t padded_size = (needed + 63) / 64 * 64;
    if (padded_size + query_overhead > 4096)
        throw Error("DNSCRYPT_SIZE", "Encrypted DNSCrypt query exceeds 4096 bytes");
    Packet padded(padded_size);
    std::copy(request.begin(), request.end(), padded.begin());
    padded[request.size()] = 0x80;
    ...
    Packet result;
    result.reserve(query_overhead + padded.size());
    ...
    const size_t encrypted_at = result.size();
    result.resize(result.size() + padded.size() + crypto_box_curve25519xchacha20poly1305_MACBYTES);
    const int encrypted =
        certificate.encryption_system == 1
            ? crypto_box_easy_afternm(result.data() + encrypted_at,
                                      padded.data(),
                                      static_cast<unsigned long long>(padded.size()),
                                      nonce.data(),
                                      shared.data())
            : crypto_box_curve25519xchacha20poly1305_easy_afternm(
                  result.data() + encrypted_at,
                  padded.data(),
                  static_cast<unsigned long long>(padded.size()),
                  nonce.data(),
                  shared.data());
...
Packet open_dnscrypt_response(const Packet& response,
                              std::span<const uint8_t, 32> shared_key,
                              std::span<const uint8_t, 12> client_nonce,
                              uint16_t encryption_system) {
    sodium_ready();
    ...
    std::array<uint8_t, 24> nonce{};
    std::copy_n(response.begin() + 8, 24, nonce.begin());
    Packet plain(response.size() - 32 - crypto_box_curve25519xchacha20poly1305_MACBYTES);
    if (encryption_system != 1 && encryption_system != 2)
        throw Error("DNSCRYPT_CERT", "Unsupported DNSCrypt encryption system");
    const int opened =
        encryption_system == 1
            ? crypto_box_open_easy_afternm(plain.data(),
                                           response.data() + 32,
                                           static_cast<unsigned long long>(response.size() - 32),
                                           nonce.data(),
                                           shared_key.data())
    ...
    while (end && plain[end - 1] == 0)
        --end;
    if (!end || plain[end - 1] != 0x80)
        throw Error("DNSCRYPT_PADDING", "Invalid DNSCrypt response padding");
    plain.resize(end - 1);
    if (plain.size() < 12)
        throw Error("DNS_MALFORMED", "Decrypted DNSCrypt response is too short");
    return plain;
}
