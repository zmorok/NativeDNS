...
namespace nd {
namespace {
std::filesystem::path file_log_directory() {
    return platform::application_root_directory() / "logs";
}
} // namespace
CoreHost::CoreHost(Config config,
                   Server original,
                   uint16_t local_port,
    ...
    try {
        ...
        logger_.configure_file(config_.logging.file_enabled, config_.logging.file, file_log_path_);
    ...
    logger_.write(Level::verbose,
                  "CORE_PATHS",
                  "executable=" + platform::executable_path().string() +
                      " working_directory=" + std::filesystem::current_path().string() +
                      " log=" + (file_log_path_.empty() ? "disabled" : file_log_path_.string()));
    logger_.write(Level::verbose,
                  "CONFIG_SUMMARY",
                  "servers=" + std::to_string(config_.servers.size()) +
                      " rules=" + std::to_string(config_.rules.size()) + " mode=" +
                      (mode == InterceptionMode::transparent ? "transparent" : "local_proxy"));
    router_ = std::make_shared<Router>(config_, logger_);
    if (mode == InterceptionMode::transparent)
        interception_ =
            make_platform_interception(config_, logger_, local_port_ ? local_port_ : 53);
    else
        interception_ = std::make_unique<LocalProxy>(router_, original_, logger_, local_port_);
    ipc_ = std::make_unique<PipeServer>(
        pipe_name_,
        [this](IpcOperation op, const std::string& payload) { return handle(op, payload); });
    log_ipc_ = std::make_unique<PipeServer>(
        log_pipe_name_, [this](IpcOperation op, const std::string& payload) {
            if (op != IpcOperation::logs)
    ...
    try {
        ...
        try {
            ...
            logger_.write(Level::errors_only,
                          "CORE_START_FAILED",
                          std::string("stage=") + stage + " error=" + error.what());
        } catch (...) {
            logger_.write(Level::errors_only,
                          "CORE_START_FAILED",
        ...
        logger_.write(Level::verbose,
                      "CORE_START_ROLLBACK",
                      "Released resources acquired before " + std::string(stage) +
                          " startup failure");
        std::rethrow_exception(failure);
    }
}
    ...
    for (;;) {
        ...
        if (value.state == State::error)
            throw Error(value.error_code.empty() ? "INTERCEPTION_FAILED" : value.error_code,
                        value.error_message.empty() ? "Interception backend failed"
                                                    : value.error_message);
    }
}
InterceptionStatus CoreHost::status() const {
    std::lock_guard lock(mutex_);
    return interception_->status();
}
IpcResponse CoreHost::handle(IpcOperation operation, const std::string& payload) {
    if (operation != IpcOperation::logs && operation != IpcOperation::configure_file_log &&
        !payload.empty())
    ...
    if (operation == IpcOperation::status) {
        ...
        auto text = std::string(state) + " port=" + std::to_string(value.port) +
                    " transparent=" + (value.transparent ? "1" : "0") +
                    " udp=" + (value.udp ? "1" : "0") + " tcp=" + (value.tcp ? "1" : "0");
        if (!value.error_code.empty())
            text += " error=" + value.error_code + " " + value.error_message;
    ...
    if (operation == IpcOperation::logs) {
        ...
        if (level_separator != std::string::npos) {
            ...
            if (level.empty() || error != std::errc{} || end != level.data() + level.size() ||
                requested_level > 3)
                throw Error("IPC_PROTOCOL", "logs level must be 0..3");
        }
        std::ostringstream output;
        const auto level = static_cast<Level>(requested_level);
        const auto events =
            wait_ms ? logger_.wait_snapshot(level, after, std::chrono::milliseconds(wait_ms))
                    : logger_.snapshot(level, after);
    ...
    if (operation == IpcOperation::clear_file_log) {
        logger_.clear_file();
        return {0, "FILE_LOG_CLEARED"};
    }
    if (operation == IpcOperation::configure_file_log) {
        std::lock_guard lock(mutex_);
        const auto separator = payload.find('\t');
        if (separator == std::string::npos)
            throw Error("IPC_PROTOCOL", "file log payload must be enabled<TAB>level");
        const auto enabled_text = payload.substr(0, separator),
                   level_text = payload.substr(separator + 1);
        if (enabled_text != "0" && enabled_text != "1")
            throw Error("IPC_PROTOCOL", "file log enabled must be 0 or 1");
        uint64_t requested_level = 0;
        auto [end, error] = std::from_chars(
            level_text.data(), level_text.data() + level_text.size(), requested_level);
        if (level_text.empty() || error != std::errc{} ||
            end != level_text.data() + level_text.size() || requested_level > 3)
            throw Error("IPC_PROTOCOL", "file log level must be 0..3");
        const bool enabled = enabled_text == "1";
        if (enabled && !file_log_enabled_)
            file_log_path_ = timestamped_log_path(file_log_directory());
        ...
        if (enabled)
            logger_.write(Level::normal,
                          "FILE_LOG_CONFIGURED",
                          "path=" + file_log_path_.string() + " level=" + level_text);
        return {0, enabled ? "FILE_LOG_ENABLED" : "FILE_LOG_DISABLED"};
    }
    if (operation == IpcOperation::clear_display) {
        logger_.clear_display();
        return {0, "DISPLAY_LOG_CLEARED"};
    }
