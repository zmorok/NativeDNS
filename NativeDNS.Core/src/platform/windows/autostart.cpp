...
template <class T> struct ComPtr {
    ...
    ComPtr(ComPtr&& other) noexcept : value(other.value) {
        other.value = nullptr;
    }
...
struct Bstr {
    ...
    explicit Bstr(const wchar_t* text) : value(SysAllocString(text)) {
        if (!value)
            throw Error("AUTOSTART", "Cannot allocate task string");
    }
...
struct ComApartment {
    ...
    ComApartment() {
        ...
        if (SUCCEEDED(result))
            ...
            fail(result, "Initialize Task Scheduler COM");
    }
    ~ComApartment() {
        if (initialized)
            CoUninitialize();
    ...
    static void fail(HRESULT result, const char* operation) {
        const auto code =
            result == E_ACCESSDENIED || result == HRESULT_FROM_WIN32(ERROR_ACCESS_DENIED)
                ? "ELEVATION_REQUIRED"
                : "AUTOSTART";
        throw Error(code,
                    std::string(operation) +
                        " failed: " + std::to_string(static_cast<uint32_t>(result)));
    }
};
void check(HRESULT result, const char* operation) {
    if (FAILED(result))
        ComApartment::fail(result, operation);
}
...
struct Scheduler {
    ...
    Scheduler() {
        check(CoCreateInstance(
                  CLSID_TaskScheduler, nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(service.put())),
...
void enable_autostart(const std::filesystem::path& executable,
    ...
    const auto gui =
        gui_executable.empty() ? executable.parent_path() / L"NativeDNS.exe" : gui_executable;
    ...
    if (!std::filesystem::is_regular_file(gui))
        throw Error("AUTOSTART", "NativeDNS GUI executable does not exist");
    ...
    Bstr author(L"NativeDNS"), description(L"Starts the NativeDNS transparent DNS Core at user "
                                           L"logon and on demand for the desktop application.");
    ...
    check(core_triggers->Create(TASK_TRIGGER_LOGON, core_trigger.put()),
          "Create Core logon trigger");
    ComPtr<ILogonTrigger> core_logon;
    check(core_trigger->QueryInterface(IID_PPV_ARGS(core_logon.put())),
          "Configure Core logon trigger");
    ...
    const std::wstring arguments =
        L"--config " + quote_argument(absolute_config.wstring()) + L" --transparent";
    Bstr executable_text(absolute_executable.wstring()), arguments_text(arguments),
        working_directory(absolute_executable.parent_path().wstring());
    ...
    check(scheduler.root->RegisterTaskDefinition(name.value,
                                                 definition.value,
                                                 ...
                                                 empty,
                                                 registered.put()),
          "Register NativeDNS autostart task");
    ...
    check(gui_definition->get_RegistrationInfo(gui_registration.put()),
          "Open GUI task registration info");
    Bstr gui_author(L"NativeDNS"),
        gui_description(L"Starts the NativeDNS desktop application in the system tray.");
    ...
    check(gui_settings->put_MultipleInstances(TASK_INSTANCES_IGNORE_NEW),
          "Set single GUI task instance");
    check(gui_settings->put_RestartCount(3), "Set GUI restart count");
    Bstr gui_restart_interval(L"PT1M"), gui_execution_limit(L"PT0S");
    check(gui_settings->put_RestartInterval(gui_restart_interval.value),
          "Set GUI restart interval");
    check(gui_settings->put_ExecutionTimeLimit(gui_execution_limit.value),
          "Remove GUI task execution limit");
    ...
    check(gui_trigger->QueryInterface(IID_PPV_ARGS(gui_logon.put())),
          "Configure GUI logon trigger");
    ...
    check(gui_action->QueryInterface(IID_PPV_ARGS(gui_execute.put())),
          "Configure GUI executable action");
    const auto absolute_gui = std::filesystem::absolute(gui).lexically_normal();
    Bstr gui_path(absolute_gui.wstring()), gui_arguments(L"--background"),
        gui_working_directory(absolute_gui.parent_path().wstring());
    ...
    check(gui_execute->put_WorkingDirectory(gui_working_directory.value),
          "Set GUI task working directory");
    Bstr gui_name(gui_task_name);
    ComPtr<IRegisteredTask> gui_registered;
    check(scheduler.root->RegisterTaskDefinition(gui_name.value,
                                                 gui_definition.value,
                                                 TASK_CREATE_OR_UPDATE,
                                                 empty,
                                                 empty,
                                                 TASK_LOGON_INTERACTIVE_TOKEN,
                                                 empty,
                                                 gui_registered.put()),
          "Register NativeDNS GUI autostart task");
}
...
void disable_autostart() {
    ...
    if (FAILED(result) && result != HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND))
        check(result, "Delete NativeDNS autostart task");
    Bstr gui_name(gui_task_name);
    const HRESULT gui_result = scheduler.root->DeleteTask(gui_name.value, 0);
    if (FAILED(gui_result) && gui_result != HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND))
        check(gui_result, "Delete NativeDNS GUI autostart task");
}
...
AutostartStatus autostart_status() {
    ...
    result.needs_repair =
        core_enabled != VARIANT_TRUE || enabled != VARIANT_TRUE || !core_has_logon_trigger;
    ...
    if (path.value)
        result.executable = path.value;
    if (arguments.value)
        result.arguments = narrow(arguments.value);
    return result;
}
...
bool start_autostart_core(const std::filesystem::path& executable,
    ...
    ComPtr<IExecAction> execute;
    check(action->QueryInterface(IID_PPV_ARGS(execute.put())),
          "Read NativeDNS Core task executable");
    OutBstr path, arguments;
    ...
    const auto absolute_executable = std::filesystem::absolute(executable).lexically_normal();
    const auto absolute_config = std::filesystem::absolute(config).lexically_normal();
    const std::wstring expected_arguments =
        L"--config " + quote_argument(absolute_config.wstring()) + L" --transparent";
    if (!path.value || !arguments.value || _wcsicmp(path.value, absolute_executable.c_str()) != 0 ||
        expected_arguments != arguments.value)
        return false;
