...
struct Bstr {
    ...
    explicit Bstr(const wchar_t* text) : value(SysAllocString(text)) {
        if (!value)
            throw Error("FIREWALL", "Cannot allocate firewall string");
    }
...
ComPtr<INetFwRules> rules() {
    ...
    check(CoCreateInstance(
              __uuidof(NetFwPolicy2), nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(policy.put())),
...
std::wstring executable_path() {
    ...
    if (!length || length >= path.size())
        throw Error("FIREWALL", "Cannot locate NativeDNS executable");
    path.resize(length);
    return path;
}
} // namespace
...
FirewallPortRule::FirewallPortRule() {
    ...
    if (SUCCEEDED(result))
        ...
        check(result, "Initialize COM for firewall");
...
void FirewallPortRule::enable(uint16_t local_port) {
    if (!local_port)
        throw Error("FIREWALL", "Cannot allow TCP port zero");
    disable();
    auto collection = rules();
    Bstr name(rule_name);
    const HRESULT removed = collection->Remove(name.value);
    if (FAILED(removed) && removed != HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND))
        check(removed, "Remove stale firewall rule");
    ComPtr<INetFwRule> rule;
    check(CoCreateInstance(
              __uuidof(NetFwRule), nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(rule.put())),
          "Create firewall rule");
    const auto path = executable_path();
    const auto port = std::to_wstring(local_port);
    Bstr description(L"Allows only the ephemeral NativeDNS reflected TCP listener while "
                     L"transparent DNS interception is running.");
