...
struct LocalProxy::Impl {
    ...
    Impl(std::shared_ptr<const Router> r, Server o, Logger& l, uint16_t p)
        ...
        if (original.protocol != Protocol::udp && original.protocol != Protocol::tcp)
            throw Error("CONFIG", "Original fallback path must be plain DNS");
        auto cfg = default_config();
        auto server = original;
        server.id = 1;
    ...
    void cleanup() {
        ...
        if (udp_event != WSA_INVALID_EVENT) {
            WSACloseEvent(udp_event);
            udp_event = WSA_INVALID_EVENT;
        }
        if (tcp_event != WSA_INVALID_EVENT) {
            WSACloseEvent(tcp_event);
            tcp_event = WSA_INVALID_EVENT;
        }
        if (stop_event) {
    ...
    bool wait(HANDLE event) const {
        ...
        if (result != WAIT_OBJECT_0 + 1)
            throw Error("PROXY_IO", "Socket event wait failed");
        return true;
    }
    Packet process(const Packet& request) {
        auto result = router->route(request, original);
        if (result.disposition == Disposition::silent_drop)
            return {};
    ...
    void udp_loop() {
        try {
            while (wait(udp_event)) {
                ...
                if (WSAEnumNetworkEvents(udp, udp_event, &events))
                    throw Error("PROXY_IO", "UDP event enumeration failed");
                if (!(events.lNetworkEvents & FD_READ))
                    continue;
                Packet packet(65535);
                sockaddr_storage client{};
                int size = sizeof(client);
                const int n = recvfrom(udp,
                ...
                if (n == SOCKET_ERROR) {
                    if (WSAGetLastError() == WSAEWOULDBLOCK)
                        continue;
                    throw Error("PROXY_IO", "UDP receive failed");
                }
                packet.resize(static_cast<size_t>(n));
                if (!udp_handlers.submit(
                        [this, packet = std::move(packet), client, size]() mutable {
                            handle_udp(std::move(packet), client, size);
                        }))
    ...
    void transfer(SOCKET socket,
        ...
        while (at < length) {
            ...
            const int n = send_bytes ? send(socket,
                                            ...
                                            static_cast<int>(length - at),
                                            0)
                                     : recv(socket,
                                            reinterpret_cast<char*>(bytes + at),
                                            static_cast<int>(length - at),
                                            0);
            if (n == SOCKET_ERROR && WSAGetLastError() == WSAEWOULDBLOCK)
                continue;
            if (n <= 0)
                throw Error("DNS_EOF", "Client TCP frame incomplete");
    ...
    void handle_client(SOCKET client) {
        ...
        try {
            ...
            uint8_t prefix[2]{};
            transfer(client, prefix, 2, false, deadline);
            ...
            transfer(client, request.data(), request.size(), false, deadline);
            auto response = process(request);
            if (!response.empty()) {
                Packet frame{static_cast<uint8_t>(response.size() >> 8),
    ...
    void tcp_loop() {
        try {
            while (wait(tcp_event)) {
                WSANETWORKEVENTS events{};
                if (WSAEnumNetworkEvents(tcp, tcp_event, &events))
                    throw Error("PROXY_IO", "TCP event enumeration failed");
                if (!(events.lNetworkEvents & FD_ACCEPT))
                    continue;
                if (events.iErrorCode[FD_ACCEPT_BIT])
                    throw Error("PROXY_IO",
                                "TCP accept event failed: " +
                                    std::to_string(events.iErrorCode[FD_ACCEPT_BIT]));
                for (;;) {
...
void LocalProxy::start() {
    ...
    try {
        ...
        p.winsock = true;
        p.stop_event = CreateEventW(nullptr, TRUE, FALSE, nullptr);
        p.udp_event = WSACreateEvent();
        p.tcp_event = WSACreateEvent();
        if (!p.stop_event || p.udp_event == WSA_INVALID_EVENT || p.tcp_event == WSA_INVALID_EVENT)
            throw Error("PROXY_IO", "Cannot create proxy events");
        int last_bind_error = 0;
        const unsigned attempts = p.requested_port ? 1u : 128u;
        for (unsigned attempt = 0; attempt < attempts; ++attempt) {
            p.udp = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
            p.tcp = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
            if (p.udp == INVALID_SOCKET || p.tcp == INVALID_SOCKET)
                throw Error("PROXY_IO", "Cannot create listeners");
            BOOL exclusive = TRUE;
            if (setsockopt(p.udp,
                           SOL_SOCKET,
                           SO_EXCLUSIVEADDRUSE,
            ...
            if (!p.requested_port && BCryptGenRandom(nullptr,
                                                     reinterpret_cast<PUCHAR>(&random_port),
                                                     sizeof(random_port),
                                                     BCRYPT_USE_SYSTEM_PREFERRED_RNG) < 0)
                throw Error("PROXY_IO", "Cannot choose a secure random listener port");
            const uint16_t chosen = p.requested_port
                                        ? p.requested_port
                                        : static_cast<uint16_t>(20000 + (random_port % 28000));
            sockaddr_in address{};
            address.sin_family = AF_INET;
            address.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
            address.sin_port = htons(chosen);
            if (bind(p.tcp, reinterpret_cast<sockaddr*>(&address), sizeof(address))) {
                last_bind_error = WSAGetLastError();
                closesocket(p.udp);
                closesocket(p.tcp);
                p.udp = INVALID_SOCKET;
                p.tcp = INVALID_SOCKET;
                if (p.requested_port)
                    throw Error("PROXY_BIND",
                                "TCP bind failed: " + std::to_string(last_bind_error));
                continue;
            }
            int size = sizeof(address);
            if (getsockname(p.tcp, reinterpret_cast<sockaddr*>(&address), &size))
                throw Error("PROXY_IO", "Cannot inspect local port");
            const auto candidate = ntohs(address.sin_port);
            if ((p.original.ip == "127.0.0.1" || p.original.ip == "::1") &&
                (p.original.port ? p.original.port : 53) == candidate) {
                closesocket(p.udp);
                closesocket(p.tcp);
                p.udp = INVALID_SOCKET;
                p.tcp = INVALID_SOCKET;
                if (p.requested_port)
                    throw Error("LOOP", "Original resolver points to local proxy");
                continue;
            }
            if (!listen(p.tcp, SOMAXCONN) &&
                !bind(p.udp, reinterpret_cast<sockaddr*>(&address), sizeof(address))) {
                p.bound_port = candidate;
                break;
            }
            last_bind_error = WSAGetLastError();
            closesocket(p.udp);
        ...
        if (p.udp == INVALID_SOCKET || p.tcp == INVALID_SOCKET)
            throw Error("PROXY_BIND",
                        "Cannot bind UDP/TCP listener pair: " + std::to_string(last_bind_error));
        if (WSAEventSelect(p.udp, p.udp_event, FD_READ) ||
            WSAEventSelect(p.tcp, p.tcp_event, FD_ACCEPT))
            throw Error("PROXY_IO", "Cannot register socket events");
        p.udp_handlers.start(8, 1024);
        p.tcp_handlers.start(16, 128);
        p.state = State::running;
        p.udp_worker = std::jthread([&p] { p.udp_loop(); });
        p.tcp_worker = std::jthread([&p] { p.tcp_loop(); });
    } catch (...) {
        if (p.stop_event)
            SetEvent(p.stop_event);
        if (p.udp_worker.joinable())
            p.udp_worker.join();
        if (p.tcp_worker.joinable())
            p.tcp_worker.join();
        p.udp_handlers.stop();
        p.tcp_handlers.stop();
        p.cleanup();
