...
uint32_t get32(const uint8_t* in) {
    uint32_t v = 0;
    for (unsigned i = 0; i < 4; ++i)
        v |= static_cast<uint32_t>(in[i]) << (i * 8);
    return v;
}
std::vector<uint8_t>
frame(uint16_t operation, uint64_t request, uint32_t status, const std::string& payload) {
    ...
    put32(out.data(), magic);
    put16(out.data() + 4, 1);
    put16(out.data() + 6, operation);
    put64(out.data() + 8, request);
    put32(out.data() + 16, static_cast<uint32_t>(payload.size()));
    ...
    if (operation & 0x8000) {
        ...
        std::copy(out.begin(), out.begin() + header_size, response.begin());
        put32(response.data() + 16, static_cast<uint32_t>(payload.size() + 4));
        put32(response.data() + header_size, status);
        std::copy(payload.begin(), payload.end(), response.begin() + header_size + 4);
        return response;
    }
...
void timed_transfer(HANDLE pipe, uint8_t* data, size_t size, bool write, uint64_t deadline) {
    ...
    while (at < size) {
        ...
        if (!event)
            throw Error("IPC_IO", "Cannot create client IPC event");
        OVERLAPPED overlapped{};
        overlapped.hEvent = event;
        DWORD amount = 0;
        BOOL ok =
            write ? WriteFile(pipe, data + at, static_cast<DWORD>(size - at), nullptr, &overlapped)
        ...
        const auto remaining =
            now >= deadline ? 0 : static_cast<DWORD>(std::min<uint64_t>(deadline - now, MAXDWORD));
        ...
        if (!GetOverlappedResult(pipe, &overlapped, &amount, FALSE) || !amount) {
            ...
            CloseHandle(event);
            throw Error("IPC_IO",
                        "Named pipe transfer completion failed: " + std::to_string(error));
        }
        CloseHandle(event);
        at += amount;
    }
...
bool async_transfer(HANDLE pipe, uint8_t* data, size_t size, bool write, HANDLE stop) {
    ...
    while (at < size) {
        HANDLE event = CreateEventW(nullptr, TRUE, FALSE, nullptr);
        if (!event)
            throw Error("IPC_IO", "Cannot create IPC event");
        OVERLAPPED overlapped{};
        overlapped.hEvent = event;
        DWORD amount = 0;
        BOOL ok =
        ...
        if (wait != WAIT_OBJECT_0 + 1 || !GetOverlappedResult(pipe, &overlapped, &amount, FALSE) ||
            ...
            const auto error = GetLastError();
            CloseHandle(event);
            throw Error("IPC_IO", "Named pipe completion: " + std::to_string(error));
        }
        CloseHandle(event);
        at += amount;
    }
...
PSECURITY_DESCRIPTOR current_user_descriptor() {
    ...
    if (!OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &token))
        throw Error("IPC_SECURITY", "Cannot open process token");
    DWORD size = 0;
    GetTokenInformation(token, TokenUser, nullptr, 0, &size);
    std::vector<uint8_t> buffer(size);
    if (!GetTokenInformation(token, TokenUser, buffer.data(), size, &size)) {
        CloseHandle(token);
        throw Error("IPC_SECURITY", "Cannot read user token");
    }
    CloseHandle(token);
    wchar_t* sid = nullptr;
    if (!ConvertSidToStringSidW(reinterpret_cast<TOKEN_USER*>(buffer.data())->User.Sid, &sid))
        throw Error("IPC_SECURITY", "Cannot format user SID");
    const std::wstring sddl = L"D:P(A;;GA;;;" + std::wstring(sid) + L")";
    LocalFree(sid);
    PSECURITY_DESCRIPTOR descriptor = nullptr;
    if (!ConvertStringSecurityDescriptorToSecurityDescriptorW(
            sddl.c_str(), SDDL_REVISION_1, &descriptor, nullptr))
        throw Error("IPC_SECURITY", "Cannot create pipe ACL");
    return descriptor;
}
} // namespace
PipeServer::PipeServer(std::string name, Handler handler)
    ...
    if (name_.empty() || !handler_)
        throw Error("IPC_CONFIG", "Pipe server needs name and handler");
}
...
void PipeServer::start() {
    if (running_.exchange(true))
        throw Error("IPC_LIFECYCLE", "Pipe server already running");
    ...
    if (!event || !startup) {
        ...
        if (startup)
            CloseHandle(startup);
        running_ = false;
        throw Error("IPC_IO", "Cannot create pipe lifecycle events");
    }
    {
    ...
    native_stop_ = event;
    startup_event_ = startup;
    thread_ = std::jthread([this] { run(); });
    if (WaitForSingleObject(startup, 3000) != WAIT_OBJECT_0) {
        stop();
        throw Error("IPC_IO", "Timed out creating named pipe");
    }
    if (!running_) {
        std::string error_text;
        {
            std::lock_guard lock(error_mutex_);
            error_text = startup_error_;
        }
        stop();
        throw Error("IPC_IO", "Named pipe startup failed: " + error_text);
    }
}
void PipeServer::stop() {
    ...
    if (thread_.joinable())
        thread_.join();
    if (native_stop_)
        CloseHandle(static_cast<HANDLE>(native_stop_));
    native_stop_ = nullptr;
    if (startup_event_)
    ...
    try {
        ...
        while (running_) {
            ...
            if (pipe == INVALID_HANDLE_VALUE)
                throw Error("IPC_IO", "CreateNamedPipe: " + std::to_string(GetLastError()));
            if (!startup_signaled) {
            ...
            if (!connected) {
                CloseHandle(pipe);
                throw Error("IPC_IO", "Cannot create connect event");
            }
            OVERLAPPED overlap{};
...
IpcResponse pipe_request(const std::string& name,
    ...
    for (;;) {
        pipe = CreateFileW(widen(name).c_str(),
        ...
        if (error != ERROR_FILE_NOT_FOUND && error != ERROR_PIPE_BUSY)
            throw Error("IPC_CONNECT", "Cannot connect core host: " + std::to_string(error));
        const auto now = GetTickCount64();
        if (now >= deadline)
            throw Error("IPC_CONNECT", "Core host pipe unavailable: " + std::to_string(error));
        const auto wait = static_cast<DWORD>(std::min<uint64_t>(deadline - now, 50));
        if (error == ERROR_PIPE_BUSY)
            (void)WaitNamedPipeW(widen(name).c_str(), wait);
        else
            Sleep(wait);
    }
    try {
        ...
        timed_transfer(pipe, output.data(), output.size(), true, deadline);
        uint8_t header[header_size]{};
        timed_transfer(pipe, header, sizeof(header), false, deadline);
        ...
        timed_transfer(pipe, body.data(), body.size(), false, deadline);
        IpcResponse response;
