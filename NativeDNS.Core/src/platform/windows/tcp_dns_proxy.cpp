        ...
        if (result.disposition == Disposition::forward_original) {
            if (logger.enabled(Level::normal))
                logger.write(Level::normal,
                             "DNS_BYPASS",
                             "Forwarding TCP request to original destination " + original.ip);
        ...
        try {
            ...
            if (!consume_expected(original.ip, client_port)) {
                logger.write(Level::errors_only,
                             "TCP_PROXY_REJECT",
                             "Rejected connection without a captured SYN");
                return;
            }
        ...
        try {
            while (running) {
                ...
                if (client == INVALID_SOCKET) {
                    ...
                    logger.write(Level::errors_only,
                                 "TCP_PROXY_IO",
                                 "TCP proxy accept failed: " + std::to_string(WSAGetLastError()));
                    continue;
                }
                ...
                if (!handlers.submit(
                        [this, owned = std::move(owned), peer] { handle(owned, peer); })) {
                    {
                        std::lock_guard lock(clients_mutex);
                        clients.erase(client);
                    }
                    logger.write(Level::errors_only,
                                 "TCP_PROXY_BUSY",
                                 "Transparent TCP proxy connection limit reached");
                }
            }
        } catch (const std::exception& error) {
            if (running)
...
uint16_t TcpDnsProxy::start() {
    ...
    if (p.running.exchange(true))
        throw Error("LIFECYCLE", "TCP DNS proxy already running");
    try {
        WSADATA data{};
        if (WSAStartup(MAKEWORD(2, 2), &data))
            throw Error("SOCKET_INIT", "TCP proxy WSAStartup failed");
        p.winsock = true;
        int last_error = 0;
        for (unsigned attempt = 0; attempt < 128; ++attempt) {
            ...
            if (p.ipv4 == INVALID_SOCKET || p.ipv6 == INVALID_SOCKET)
                throw Error("TCP_PROXY_IO", "Cannot create TCP proxy listeners");
            BOOL exclusive = TRUE, ipv6_only = TRUE;
            if (setsockopt(p.ipv4,
                           SOL_SOCKET,
                           SO_EXCLUSIVEADDRUSE,
            ...
            address4.sin_port = htons(candidate);
            address4.sin_addr.s_addr = htonl(INADDR_ANY);
            sockaddr_in6 address6{};
            address6.sin6_family = AF_INET6;
            address6.sin6_port = htons(candidate);
            ...
            if (!bind(p.ipv4, reinterpret_cast<sockaddr*>(&address4), sizeof(address4)) &&
                ...
                !listen(p.ipv4, 64) && !listen(p.ipv6, 64)) {
                p.port = candidate;
                break;
            }
        ...
        if (!p.port)
            throw Error("TCP_PROXY_BIND",
                        "Cannot bind TCP proxy listener pair: " + std::to_string(last_error));
        ...
        p.running = false;
        p.close_listeners();
        if (p.accept4.joinable())
            p.accept4.join();
        if (p.accept6.joinable())
            p.accept6.join();
        p.handlers.stop();
        if (p.winsock) {
            WSACleanup();
...
void TcpDnsProxy::expect_connection(std::string original_ip, uint16_t client_port) {
    if (original_ip.empty() || !client_port)
        throw Error("TCP_PROXY_IO", "Invalid expected TCP connection");
    auto& p = *impl_;
    std::lock_guard lock(p.expected_mutex);
