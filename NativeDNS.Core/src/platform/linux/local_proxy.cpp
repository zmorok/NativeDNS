...
void sys_error(const char* what) {
    throw Error("PROXY_IO", std::string(what) + ": " + std::strerror(errno));
}

void set_timeout(int fd, int seconds = 2) {
    timeval tv{seconds, 0};

    if (setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv)) < 0 ||
        setsockopt(fd, SOL_SOCKET, SO_SNDTIMEO, &tv, sizeof(tv)) < 0) {
        sys_error("setsockopt timeout");
    }
}

bool recv_exact(int fd, uint8_t* data, size_t size, bool clean_eof = false) {
    size_t at = 0;

    while (at < size) {
        const auto n = recv(fd, data + at, size - at, 0);
...
void send_exact(int fd, const uint8_t* data, size_t size) {
    size_t at = 0;

    while (at < size) {
        const auto n = send(fd, data + at, size - at, MSG_NOSIGNAL);
        ...
        if (!n) {
            throw Error("DNS_EOF", "Client connection closed");
        }
...
void v6_only(int fd) {
    ...
    if (setsockopt(fd, IPPROTO_IPV6, IPV6_V6ONLY, &yes, sizeof(yes)) < 0) {
        sys_error("IPV6_V6ONLY");
    }
}

bool wait_readable(int fd, const std::atomic_bool& running) {
...
struct LocalProxy::Impl {
    ...
    Fd udp6;
    Fd tcp6;
    std::jthread udp4_worker;
    std::jthread tcp4_worker;
    std::jthread udp6_worker;
    ...
    void handle_udp(int fd, Packet packet, sockaddr_storage client, socklen_t size) {
        try {
            ...
            logger.write(Level::errors_only, e.code, e.what());
        } catch (const std::exception& e) {
            logger.write(Level::errors_only, "PROXY_IO", e.what());
        }
    }
...
void LocalProxy::start() {
    ...
    try {
        ...
        for (unsigned attempt = 0; attempt < attempts; ++attempt) {
            p.udp4.reset(socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP));
            p.tcp4.reset(socket(AF_INET, SOCK_STREAM, IPPROTO_TCP));
            ...
            reuse(p.udp6.value);
            reuse(p.tcp6.value);
            v6_only(p.udp6.value);
            v6_only(p.tcp6.value);
        ...
        const int udp4_fd = p.udp4.value;
        const int tcp4_fd = p.tcp4.value;
