...
struct Socket {
    int value = -1;
    explicit Socket(int s) : value(s) { if (s < 0) throw Error("SOCKET", "Socket creation: " + std::string(std::strerror(errno))); }
...
void socket_error(const char* operation) {
    ...
    throw Error(code == ETIMEDOUT ? "TIMEOUT" : "SOCKET", std::string(operation) + ": " + std::strerror(code));
}
void ready(int socket, bool writing, Clock::time_point deadline) {
    ...
    const int result = select(socket + 1, writing ? nullptr : &requested, writing ? &requested : nullptr, &errors, &timeout);
    if (result < 0) socket_error("select");
    if (!result) throw Error("TIMEOUT", "DNS request timed out");
    if (FD_ISSET(socket, &errors)) {
        int error = 0; socklen_t size = sizeof(error);
        if (getsockopt(socket, SOL_SOCKET, SO_ERROR, &error, &size) < 0) socket_error("getsockopt");
...
void transfer(int socket, uint8_t* bytes, size_t size, bool sending, Clock::time_point deadline) {
    ...
    while (at < size) {
        ...
        const ssize_t result = sending ? send(socket, bytes + at, size - at, MSG_NOSIGNAL)
                                      : recv(socket, bytes + at, size - at, 0);
        if (result < 0) { if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR) continue; socket_error(sending ? "send" : "recv"); }
