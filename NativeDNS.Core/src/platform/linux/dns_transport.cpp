...
struct Socket {
    int value = -1;
    explicit Socket(int s) : value(s) {
        if (s < 0)
            throw Error("SOCKET", "Socket creation: " + std::string(std::strerror(errno)));
    }
    ~Socket() {
        if (value >= 0)
            ::close(value);
...
void socket_error(const char* operation) {
    ...
    throw Error(code == ETIMEDOUT ? "TIMEOUT" : "SOCKET",
...
ConnectedSocket connect_network(const Server& server, bool tcp, Clock::time_point deadline) {
    ...
    if (bind(socket->value, reinterpret_cast<const sockaddr*>(&local), local_size) < 0)
    ...
    if (connect(socket->value, reinterpret_cast<const sockaddr*>(&address), address_size) < 0) {
        ...
        if (getsockopt(socket->value, SOL_SOCKET, SO_ERROR, &error, &length) < 0)
