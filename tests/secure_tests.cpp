...
int main(int argc, char**) {
    try {
        const auto prepare=[](nd::Server value){auto config=nd::default_config();value.id=1;config.servers.push_back(std::move(value));nd::prepare_secure_endpoints(config);return config.servers.front();};
#ifdef _WIN32
        ...
        server.enabled = true; server.url = "https://localhost/dns-query";
        check(nd::test_server(server).error_code == "BOOTSTRAP", "Implicit runtime bootstrap must be rejected");
        auto bootstrap_config=nd::default_config();server.id=1;bootstrap_config.servers.push_back(server);
        nd::prepare_secure_endpoints(bootstrap_config);
        check(!bootstrap_config.servers.front().ip.empty(),"Secure endpoint was not resolved before interception");
        if (argc > 1) {
            ...
            server.url = "https://wrong.host.badssl.com/";server.ip.clear();server=prepare(server);
            ...
            server.url = "https://xbox-dns.ru/dns-query";server.ip.clear();server=prepare(server);
            ...
            server.hashes.clear(); server.ip.clear(); server.bootstrap = {"1.1.1.1"};
