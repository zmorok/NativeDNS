...
namespace {
struct Resources {
    uint64_t memory = 0, handles = 0, threads = 0;
};
Resources resources() {
#ifdef _WIN32
    ...
    if (getrusage(RUSAGE_SELF, &usage) == 0)
        value.memory = static_cast<uint64_t>(usage.ru_maxrss) * 1024;
    return value;
#endif
}
bool exact(Socket socket, uint8_t* data, size_t size, bool writing) {
    size_t offset = 0;
    while (offset < size) {
        const int count = writing ? send(socket,
...
Socket connect_to(uint16_t port, int type) {
    Socket socket_value = socket(AF_INET, type, type == SOCK_STREAM ? IPPROTO_TCP : IPPROTO_UDP);
    if (socket_value == bad_socket)
        return bad_socket;
#ifdef _WIN32
    DWORD timeout = 2000;
    setsockopt(socket_value,
               SOL_SOCKET,
               SO_RCVTIMEO,
    ...
    if (connect(socket_value, reinterpret_cast<sockaddr*>(&address), sizeof(address)) != 0) {
...
bool request(uint16_t port, bool tcp, const nd::Packet& query) {
    Socket client = connect_to(port, tcp ? SOCK_STREAM : SOCK_DGRAM);
    if (client == bad_socket)
        return false;
    bool ok = false;
    if (tcp) {
        std::vector<uint8_t> frame{static_cast<uint8_t>(query.size() >> 8),
                                   static_cast<uint8_t>(query.size())};
        frame.insert(frame.end(), query.begin(), query.end());
        uint8_t prefix[2]{};
        ok = exact(client, frame.data(), frame.size(), true) && exact(client, prefix, 2, false);
        if (ok) {
            const size_t size = (static_cast<size_t>(prefix[0]) << 8) | prefix[1];
            std::vector<uint8_t> response(size);
            ok = size >= 12 && exact(client, response.data(), response.size(), false);
        }
    } else {
        ok = send(client,
        ...
        if (ok)
            ok = recv(client, reinterpret_cast<char*>(response), sizeof(response), 0) >= 12;
    }
    close_socket(client);
    return ok;
}
uint64_t number(const char* text) {
    ...
    if (parsed.ec != std::errc{} || parsed.ptr != end || !value)
...
int main(int argc, char** argv) {
    try {
        uint64_t seconds = 30, clients = 16;
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (arg == "--seconds" && i + 1 < argc)
                ...
                clients = number(argv[++i]);
            else if (arg == "--debug-logging")
                debug_logging = true;
            else
                throw std::runtime_error(
                    "usage: nativedns_stress [--seconds N] [--clients N] [--debug-logging]");
        }
#ifdef _WIN32
        WSADATA sockets{};
        ...
        config.logging.screen = debug_logging ? nd::Level::debug : nd::Level::errors_only;
        nd::Logger logger(4096);
        logger.set_display_level(config.logging.screen);
        auto router = std::make_shared<nd::Router>(config, logger);
        nd::Server unused;
        unused.name = "unused";
        unused.ip = "127.0.0.1";
        unused.port = 1;
        const auto before = resources();
        nd::LocalProxy proxy(router, unused, logger);
        proxy.start();
        ...
        for (uint64_t index = 0; index < clients; ++index)
            workers.emplace_back([&, index] {
                ...
                while (std::chrono::steady_clock::now() < deadline) {
                    ...
                    const bool ok = request(port, tcp, query);
                    const auto micros =
