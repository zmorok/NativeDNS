...
void fault_rollback(const char* stage) {
    nd::Logger logger;
    nd::WinDivertInterception interception(nd::default_config(), logger);
    ...
    check(failed && interception.status().state == nd::State::error,
          "injected WinDivert startup failure");
    interception.stop();
    check(interception.status().state == nd::State::stopped,
          "WinDivert fault rollback releases resources");
}
...
uint32_t add(uint32_t sum, const uint8_t* p, size_t n) {
    while (n >= 2) {
        sum += read16(p);
        p += 2;
        n -= 2;
...
nd::Packet dns_response() {
    auto packet = nd::make_query("example.com");
    packet[2] = 0x81;
    packet[3] = 0x80;
    return packet;
}
nd::Packet ipv4_query() {
    const auto dns = nd::make_query("example.com");
    nd::Packet packet(28 + dns.size());
    packet[0] = 0x45;
    packet[8] = 64;
    packet[9] = 17;
    ...
    write16(packet.data() + 20, 50000);
    write16(packet.data() + 22, 53);
    write16(packet.data() + 24, static_cast<uint16_t>(8 + dns.size()));
    std::copy(dns.begin(), dns.end(), packet.begin() + 28);
    return packet;
}
nd::Packet ipv6_query() {
    const auto dns = nd::make_query("example.com");
    nd::Packet packet(48 + dns.size());
    packet[0] = 0x60;
    write16(packet.data() + 4, static_cast<uint16_t>(8 + dns.size()));
    packet[6] = 17;
    packet[7] = 64;
    packet[8] = 0x20;
    packet[9] = 0x01;
    ...
    packet[39] = 0x88;
    packet[38] = 0x88;
    write16(packet.data() + 40, 50001);
    write16(packet.data() + 42, 53);
    write16(packet.data() + 44, static_cast<uint16_t>(8 + dns.size()));
    std::copy(dns.begin(), dns.end(), packet.begin() + 48);
    return packet;
}
nd::Packet ipv6_extension_query() {
    auto packet = ipv6_query();
    packet.insert(packet.begin() + 40, 8, 0);
    packet[6] = 60;
    packet[40] = 17;
    packet[41] = 0;
...
nd::Packet ipv4_tcp(uint16_t source, uint16_t destination) {
    ...
    std::copy(client.begin(), client.end(), packet.begin() + 12);
    std::copy(server.begin(), server.end(), packet.begin() + 16);
    write16(packet.data() + 20, source);
    write16(packet.data() + 22, destination);
    packet[32] = 0x50;
    packet[33] = 0x18;
    packet[40] = 'd';
    packet[41] = 'n';
...
nd::Packet ipv6_tcp(uint16_t source, uint16_t destination) {
    ...
    packet[38] = 0x88;
    packet[39] = 0x88;
    write16(packet.data() + 40, source);
    write16(packet.data() + 42, destination);
    packet[52] = 0x50;
    packet[53] = 0x18;
    packet[60] = 'd';
    packet[61] = 'n';
...
void verify_tcp_checksum(const nd::Packet& packet, size_t tcp, bool ipv6) {
    ...
    if (ipv6) {
        ...
        const std::array<uint8_t, 8> pseudo{static_cast<uint8_t>(length >> 24),
        ...
        const std::array<uint8_t, 4> pseudo{
            0, 6, static_cast<uint8_t>(length >> 8), static_cast<uint8_t>(length)};
        sum = add(sum, pseudo.data(), pseudo.size());
    }
...
void tcp_proxy_roundtrip() {
    ...
    check(bind(client, reinterpret_cast<sockaddr*>(&local), sizeof(local)) == 0,
          "TCP proxy test bind");
    int local_size = sizeof(local);
    check(getsockname(client, reinterpret_cast<sockaddr*>(&local), &local_size) == 0,
          "TCP proxy test endpoint");
    ...
    for (unsigned request = 0; request < 2; ++request) {
        ...
        check(recv(client,
                   ...
                   MSG_WAITALL) == static_cast<int>(response.size()),
              "TCP proxy response frame");
        const auto parsed = nd::parse_response(response, nd::parse_question(query));
        check(parsed.addresses.size() == 1 && parsed.addresses.front() == "0.0.0.0",
    ...
    check(rejected != INVALID_SOCKET &&
          ...
          "unregistered TCP proxy connection reaches validator");
    char byte = 0;
    ...
    sockaddr_in reset_local{};
    reset_local.sin_family = AF_INET;
    reset_local.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
    check(bind(reset, reinterpret_cast<sockaddr*>(&reset_local), sizeof(reset_local)) == 0,
          "forgotten TCP proxy bind");
    int reset_size = sizeof(reset_local);
    check(getsockname(reset, reinterpret_cast<sockaddr*>(&reset_local), &reset_size) == 0,
          "forgotten TCP proxy endpoint");
    proxy.expect_connection("127.0.0.1", ntohs(reset_local.sin_port));
    proxy.forget_connection("127.0.0.1", ntohs(reset_local.sin_port));
    check(connect(reset, reinterpret_cast<sockaddr*>(&address), sizeof(address)) == 0,
          "forgotten TCP proxy connect");
    ...
    for (unsigned connection = 0; connection < 256; ++connection) {
        ...
        sockaddr_in short_local{};
        short_local.sin_family = AF_INET;
        short_local.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
        check(bind(short_lived, reinterpret_cast<sockaddr*>(&short_local), sizeof(short_local)) ==
                  0,
              "short-lived TCP proxy bind");
        int short_size = sizeof(short_local);
        check(getsockname(short_lived, reinterpret_cast<sockaddr*>(&short_local), &short_size) == 0,
              "short-lived TCP proxy endpoint");
        proxy.expect_connection("127.0.0.1", ntohs(short_local.sin_port));
        check(connect(short_lived, reinterpret_cast<sockaddr*>(&address), sizeof(address)) == 0,
              "short-lived TCP proxy connect");
        check(send(short_lived,
                   reinterpret_cast<const char*>(frame.data()),
                   static_cast<int>(frame.size()),
                   0) == static_cast<int>(frame.size()),
              "short-lived TCP proxy query");
        uint8_t short_prefix[2]{};
        check(recv(short_lived, reinterpret_cast<char*>(short_prefix), 2, MSG_WAITALL) == 2,
              "short-lived TCP proxy prefix");
        nd::Packet short_response(static_cast<size_t>((short_prefix[0] << 8) | short_prefix[1]));
        check(recv(short_lived,
                   reinterpret_cast<char*>(short_response.data()),
                   static_cast<int>(short_response.size()),
                   MSG_WAITALL) == static_cast<int>(short_response.size()),
              "short-lived TCP proxy response");
...
int main() {
    try {
        check(!nd::detail::is_network_upstream(false, 42424), "upstream registry initially empty");
        {
            ...
            check(nd::detail::is_network_upstream(false, 42424) &&
                      !nd::detail::is_network_upstream(true, 42424),
        ...
        check(!nd::should_reinject_udp_immediately(fast_path, ipv4_query()),
              "Custom Process remains on worker queue");

        tcp_proxy_roundtrip();
        const auto dns = dns_response();
        const auto v4 = nd::make_intercepted_udp_response(ipv4_query(), dns);
        check(v4[0] == 0x45 && read16(v4.data() + 2) == v4.size(), "IPv4 length");
        check(std::equal(
                  v4.begin() + 12, v4.begin() + 16, std::array<uint8_t, 4>{8, 8, 8, 8}.begin()),
              "IPv4 source swap");
        check(std::equal(
                  v4.begin() + 16, v4.begin() + 20, std::array<uint8_t, 4>{192, 0, 2, 10}.begin()),
              "IPv4 destination swap");
        ...
        uint32_t sum = add(0, v4.data() + 12, 8);
        const std::array<uint8_t, 4> pseudo4{0, 17, v4[24], v4[25]};
        sum = add(sum, pseudo4.data(), pseudo4.size());
        sum = add(sum, v4.data() + 20, v4.size() - 20);
        ...
        check(v6[8] == 0x20 && v6[9] == 0x01 && v6[10] == 0x48 && v6[11] == 0x60,
              "IPv6 source swap");
        check(v6[24] == 0x20 && v6[25] == 0x01 && v6[26] == 0x0d && v6[27] == 0xb8,
              "IPv6 destination swap");
        check(read16(v6.data() + 40) == 53 && read16(v6.data() + 42) == 50001, "IPv6 port swap");
        sum = add(0, v6.data() + 8, 32);
        const std::array<uint8_t, 8> pseudo6{0, 0, v6[44], v6[45], 0, 0, 0, 17};
        sum = add(sum, pseudo6.data(), pseudo6.size());
        sum = add(sum, v6.data() + 40, v6.size() - 40);
        ...
        check(v6_extension[6] == 60 && v6_extension[40] == 17 &&
                  read16(v6_extension.data() + 48) == 53 &&
                  read16(v6_extension.data() + 50) == 50001,
              "IPv6 destination-options DNS response");
        sum = add(0, v6_extension.data() + 8, 32);
        const std::array<uint8_t, 8> extension_pseudo{
            0, 0, v6_extension[52], v6_extension[53], 0, 0, 0, 17};
        sum = add(sum, extension_pseudo.data(), extension_pseudo.size());
        sum = add(sum, v6_extension.data() + 48, v6_extension.size() - 48);
        ...
        check(std::equal(
                  tcp4.begin() + 12, tcp4.begin() + 16, std::array<uint8_t, 4>{8, 8, 8, 8}.begin()),
              "IPv4 TCP destination reflection source");
        check(read16(tcp4.data() + 20) == 51000 && read16(tcp4.data() + 22) == 34010,
              "IPv4 TCP proxy destination");
        ...
        check(read16(tcp4_reply.data() + 20) == 53 && read16(tcp4_reply.data() + 22) == 51000,
              "IPv4 TCP DNS response source");
        verify_tcp_checksum(tcp4_reply, 20, false);

        const auto tcp6 = nd::make_reflected_tcp_packet(ipv6_tcp(51001, 53), 34010, true);
        check(tcp6[8] == 0x20 && tcp6[10] == 0x48 && read16(tcp6.data() + 40) == 51001 &&
                  read16(tcp6.data() + 42) == 34010,
              "IPv6 TCP reflection");
        ...
        fragment[6] = 0x20;
        try {
            (void)nd::make_intercepted_udp_response(fragment, dns);
        } catch (const nd::Error& error) {
            failed = error.code == "INTERCEPT_PACKET";
        }
        check(failed, "fragment rejection");
        failed = false;
        auto wrong = ipv4_query();
        wrong[23] = 54;
        try {
            (void)nd::make_intercepted_udp_response(wrong, dns);
